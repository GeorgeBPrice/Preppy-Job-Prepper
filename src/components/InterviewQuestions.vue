  ...
  <div class="interview-questions">
    ...
    <div class="questions-container">
      ...
      <div v-else class="no-questions">
        <p>Select a section to see interview questions.</p>
      </div>
    </div>
  </div>
</template>
...
onMounted(() => {
  if (props.sectionParam) {
    const sectionIndex = sections.findIndex((s) => s.id === props.sectionParam)
    if (sectionIndex >= 0) {
      ...
      if (props.questionParam) {
        ...
        if (questionIndex >= 0 && questionIndex < sections[sectionIndex].questions.length) {
...
watch(
  ...
  (newValue) => {
    if (newValue) {
      const sectionIndex = sections.findIndex((s) => s.id === newValue)
...
const selectSection = (index) => {
  if (activeSection.value !== index) {
    ...
    emit('section-change', sections[index].id)
  }
}
...
const toggleQuestion = (index) => {
  ...
  // Update the expanded state - corrected line
  expandedQuestions[activeSection.value] = newValue

  // Emit question change event
  emit('question-change', sections[activeSection.value].id, newValue)
}
</script>
